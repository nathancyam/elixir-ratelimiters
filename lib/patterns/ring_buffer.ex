defmodule Patterns.RingBuffer do
  ...
  they are a fix-sized array or linked list that you iterate through
  monotonically, wrapping around to the beginning when you are at
  the limit. Our ring buffer will hold timestamps and should be
  initialized to hold 0's ensuring that only someone with a time machine
  could be rate limited before they send any requests. The size of the buffer
  is the rate limit's value, expressed in whatever time unit you find
  convenient.
  """
  ...
  def handle_call({:enqueue, worker_fun}, _from, state) do
    ...
    buffer_value = Enum.at(state.ring, state.current_index)
    ...
    if now - rate_limit_interval < buffer_value do
      ...
      new_state =
        ...
        |> Map.update!(:ring, fn old_ring ->
          List.update_at(old_ring, state.current_index, fn _ -> now end)
        end)
